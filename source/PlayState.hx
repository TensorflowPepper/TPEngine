...
class PlayState extends MusicBeatState
{
	...
	public static var curSongDifficultyLists:Array<String> = [];
	public static var curBlueBalled:Int = 0;
	public static var botPlayWatermark:FlxText;
	public static var videoFinished:Bool = false;
	...
	private var logoWatermark:FlxText;
	#if desktop
	...
	private var isAbleToCheckTime:Bool = false;
	...
	override public function create()
	{
		...
		for (i in 3...9) {
			...
			if (curAdd) {
				switch (i - 3) {
					case 0: curSongDifficultyLists.insert(curSongDifficultyLists.length, "Easy");
					case 1: curSongDifficultyLists.insert(curSongDifficultyLists.length, "Normal");
					case 2: curSongDifficultyLists.insert(curSongDifficultyLists.length, "Hard");
					case 3: curSongDifficultyLists.insert(curSongDifficultyLists.length, "Expert");
					case 4: curSongDifficultyLists.insert(curSongDifficultyLists.length, "Insane");
				}
			}
		}

		curSongDifficultyLists.insert(curSongDifficultyLists.length, "Back");

		trace (curSongDifficultyLists.toString());
		...
		watermark = new FlxText(40, healthBarBG.y + 50,0, SONG.song.replace('-', ' ') + " (" + CoolUtil.difficultyString() + ")", 64);
		watermark.color = ToolShit.formatColor(CoolUtil.difficultyString());
		...
		logoWatermark = new FlxText(40, healthBarBG.y + 50,0, "TPEngine Version. " + MainMenuState.version, 64);
		logoWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		logoWatermark.scrollFactor.set();
		add(logoWatermark);

		logoWatermark.y = watermark.height;
		logoWatermark.screenCenter(X);

		#if desktop
		timeWatermark = new FlxText(40, healthBarBG.y + 50,0,"", 64);
		timeWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		timeWatermark.scrollFactor.set();
		add(timeWatermark);

		timeWatermark.y = logoWatermark.height + watermark.height;
		timeWatermark.screenCenter(X);
		#end

		botPlayWatermark = new FlxText(40, healthBarBG.y + 50,0, "BotPlaying", 64);
		botPlayWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		botPlayWatermark.scrollFactor.set();
		add(botPlayWatermark);

		botPlayWatermark.screenCenter(X);
		botPlayWatermark.screenCenter(Y);
		botPlayWatermark.visible = false;

		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			'health', 0, 2);
		healthBar.scrollFactor.set();
		healthBar.createFilledBar(FlxColor.fromString(curSongData[0]), FlxColor.fromString(curSongData[1]));
		// healthBar
		add(healthBar);
		...
		logoWatermark.cameras = [camHUD];
		#if desktop
		timeWatermark.cameras = [camHUD];
		#end
		botPlayWatermark.cameras = [camHUD];
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var songData = SONG;
		Conductor.changeBPM(Std.int(songData.bpm));
	...
	private function generateStaticArrows(player:Int, i:Int):Void
	{
		var babyArrow:FlxSprite = new FlxSprite(0, strumLine.y);

		var noteTypeCheck:String;
	...
	override public function update(elapsed:Float)
	{
		...
		timeWatermark.y = logoWatermark.height + watermark.height;
		timeWatermark.screenCenter(X);
		if (isAbleToCheckTime) {
			timeWatermark.text = ToolShit.formatTime(Std.int((songLength - Conductor.songPosition) / 1000)) + " Left";
		}
		#end

		if (botPlay) {
			botPlayWatermark.text = "BotPlaying";
			botPlayWatermark.visible = true;
		} else botPlayWatermark.visible = false;
	...
	function endSong():Void
	{
		...
		{
			#if !switch
			Highscore.saveScore(SONG.song.replace(' ', '-'), songScore, storyDifficulty);
			#end
		}
